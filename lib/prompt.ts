...
- 꼬리질문도 별도 Q&A로 분리
- 질문은 원문 의도를 그대로 유지
- **답변은 핵심 내용·구체적 사례·맥락을 보존하되, 같은 내용의 반복이나 장황한 부분은 간결하게 압축** (원문의 의미와 핵심 사례는 빠짐없이 유지하면서, 중복 표현 제거로 분량을 줄이기)
- **모든 질문을 포함하는 것이 최우선. 출력 길이 한계가 가까우면 답변을 더 간결하게 요약하되 질문은 절대 생략하지 말 것**
