...
export function formatForSummary(report: EvaluationReport): string {
  ...
  if (report.overall) {
    lines.push(`강점: ${report.overall.strengths.slice(0, 2).join(', ')}`);
    lines.push(`리스크: ${report.overall.risks.slice(0, 2).join(', ')}`);
  }
  return lines.join('\n');
}

// 공유용 요약 형식 (기본정보 + 링크 + 최종의견 + 면접관 소견)
export function formatForShareSummary(report: EvaluationReport, reportUrl?: string): string {
  ...
  // 기본 정보
  lines.push(`• Hiring Manager: ${interviewerName}`);
  if (reportAuthor) {
